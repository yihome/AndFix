...
extern jboolean __attribute__ ((visibility ("hidden"))) dalvik_setup(
	...
	if (dvm_hand) {
	    void* fnPtr = dvm_dlsym(dvm_hand,
                      				apilevel > 10 ?
                      						"_Z20dvmDecodeIndirectRefP6ThreadP8_jobject" :
                      						"dvmDecodeIndirectRef");
		dvmDecodeIndirectRef_fnPtr = reinterpret_cast<dvmDecodeIndirectRef_func> (fnPtr);
		if (!dvmDecodeIndirectRef_fnPtr) {
			return JNI_FALSE;
		}
		fnPtr = dvm_dlsym(dvm_hand,
                				apilevel > 10 ? "_Z13dvmThreadSelfv" : "dvmThreadSelf");
		dvmThreadSelf_fnPtr = reinterpret_cast<dvmThreadSelf_func> (fnPtr);
